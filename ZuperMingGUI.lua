...
local Custom = {} do
  ...
  function Custom:GetIconAsset(input)
  if type(input) ~= "string" then return "" end
    if input:match("^rbxassetid://") or input:match("^https?://") then
      return input
    end
    return Custom.LucideIcons[input] or ""
  end
...
local function OpenClose()
  ...
  local Close_ImageButton = Custom:Create("ImageButton", {
    ...
    Image = "rbxassetid://112448522856900",
    Visible = false,
  }, ScreenGui)
...
function ZuperMing:CreateWindow(Config)
  ...
  function Tabs:CreateTab(Config)
    ...
    local iconAsset = Custom:GetIconAsset(Icon)
    ...
    function Sections:AddSection(Title, OpenSection)
      ...
      function Item:AddKeybind(Config)
        ...
        local InputType  = Config[4] or Config.InputType or "Keyboard"
        ...
        local function GetLabel(Value)
          if typeof(Value) == "EnumItem" then
            ...
            elseif Value.EnumType == Enum.UserInputType then
              return MouseButtonNames[Value] or tostring(Value.Name)
            end
          end
          return "None"
        end

        local Funcs_Keybind = {Value = Default, Listening = false, InputType = InputType}
        ...
        UserInputService.InputBegan:Connect(function(Input, gameProcessed)
          if Funcs_Keybind.Listening then
            if InputType == "Keyboard" and Input.UserInputType == Enum.UserInputType.Keyboard then
              if Input.KeyCode ~= Enum.KeyCode.Unknown then
                Funcs_Keybind.Value = Input.KeyCode
                KeybindLabel.Text = GetLabel(Input.KeyCode)
                SetListening(false)
              end
            elseif InputType == "MouseButton" and MouseButtonNames[Input.UserInputType] then
              Funcs_Keybind.Value = Input.UserInputType
              KeybindLabel.Text = GetLabel(Input.UserInputType)
              SetListening(false)
            end
          elseif not gameProcessed then
            if InputType == "Keyboard" and Input.KeyCode == Funcs_Keybind.Value then
              Callback(Funcs_Keybind.Value)
            elseif InputType == "MouseButton" and Input.UserInputType == Funcs_Keybind.Value then
              Callback(Funcs_Keybind.Value)
            end
          end
        end)

        function Funcs_Keybind:Set(Value)
          Funcs_Keybind.Value = Value
          KeybindLabel.Text = GetLabel(Value)
        end
