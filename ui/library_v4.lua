...
function Library:CreateWindow(cfg)
	...
	tagBg(welcomeCard, "sidebar")
	tagBorder(mk("UIStroke", { Color = theme.Border; Thickness = 1; ZIndex = 51; Parent = welcomeCard }), "border")

	local logoTileW = tagBg(mk("Frame", {
		Size = UDim2.fromOffset(48, 48); AnchorPoint = Vector2.new(0.5, 0);
		Position = UDim2.new(0.5, 0, 0, 24);
		BackgroundColor3 = theme.Accent; BackgroundTransparency = 0.82;
		BorderSizePixel = 0; ZIndex = 52; Parent = welcomeCard;
	}), "accent")
	mk("UICorner", { CornerRadius = UDim.new(0, 12); Parent = logoTileW })

	local wLogo = mkIcon(logoTileW, "zap", 26, theme.Accent, 54)
	if wLogo then wLogo.AnchorPoint = Vector2.new(0.5, 0.5); wLogo.Position = UDim2.fromScale(0.5, 0.5); tagIcon(wLogo, "accent") end

	tagText(mk("TextLabel", {
		Size = UDim2.new(1, 0, 0, 28); Position = UDim2.fromOffset(0, 80);
		BackgroundTransparency = 1; Text = "Leon X";
		Font = Enum.Font.GothamBold; TextSize = 24;
		TextColor3 = theme.Text; ZIndex = 53; Parent = welcomeCard;
	}), "text")

	local tlText = isGameMode and ("Game Mode: " .. gameName) or "Universal Execution Suite"
	tagText(mk("TextLabel", {
		Size = UDim2.new(1, -40, 0, 18); Position = UDim2.fromOffset(20, 110);
	...
	local infoData = {
		{"user",        "Author",   "leonx24"},
		{"sliders",     "Mode",     isGameMode and gameName or "Universal"},
		{"shield-check","Features", "30+ Modules Active"},
		{"smartphone",  "Mobile",   "Full Touch UI Support"},
	}
	local infoFrame = mk("Frame", {
		Size = UDim2.new(1, -40, 0, #infoData * 30 + 4);
		Position = UDim2.fromOffset(20, 138);
		BackgroundColor3 = theme.Card; BorderSizePixel = 0;
		ZIndex = 53; Parent = welcomeCard;
	}, { mk("UICorner", { CornerRadius = UDim.new(0, 10) }) })
	tagBg(infoFrame, "card")

	for i, row in ipairs(infoData) do
		local rBg = mk("Frame", {
			Size = UDim2.new(1, -4, 0, 28); Position = UDim2.fromOffset(2, 2 + (i-1) * 30);
			BackgroundColor3 = (i % 2 == 0) and theme.Elevated or theme.Surface;
			BackgroundTransparency = 0.5; BorderSizePixel = 0;
			ZIndex = 54; Parent = infoFrame;
		}, { mk("UICorner", { CornerRadius = UDim.new(0, 6) }) })
		tagBg(rBg, (i % 2 == 0) and "elevated" or "surface")

		local rIco = mkIcon(rBg, row[1], 14, theme.Accent, 55)
		if rIco then rIco.Position = UDim2.new(0, 10, 0.5, -7); tagIcon(rIco, "accent") end

		tagText(mk("TextLabel", {
			Size = UDim2.new(0.32, 0, 1, 0); Position = UDim2.fromOffset(32, 0);
		...
		tagText(mk("TextLabel", {
			Size = UDim2.new(0.6, 0, 1, 0); Position = UDim2.new(0.38, 0, 0, 0);
	...
	local enterBtn = mk("TextButton", {
		...
		Position = UDim2.fromOffset(20, 138 + #infoData * 30 + 16);
		BackgroundColor3 = theme.Accent; BorderSizePixel = 0;
		Text = ""; AutoButtonColor = false; ZIndex = 53; Parent = welcomeCard;
	}, { mk("UICorner", { CornerRadius = UDim.new(0, 10) }) })
	tagBg(enterBtn, "accent")

	local eIco = mkIcon(enterBtn, "arrow-right", 16, Color3.fromRGB(10, 10, 14), 54)
	if eIco then eIco.AnchorPoint = Vector2.new(0, 0.5); eIco.Position = UDim2.new(0.5, 42, 0.5, 0) end

	mk("TextLabel", {
		Size = UDim2.fromScale(1, 1); BackgroundTransparency = 1;
		Text = "Enter Leon X"; Font = Enum.Font.GothamBold; TextSize = 13;
		TextColor3 = Color3.fromRGB(10, 10, 14); ZIndex = 54; Parent = enterBtn;
	})
