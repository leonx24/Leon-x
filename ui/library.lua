-- ╔══════════════════════════════════════════════════════╗
-- ║         Leon X  ·  UI Library  v4.0                 ║
-- ║   Dark · Minimal · Clean · No bugs                  ║
-- ╚══════════════════════════════════════════════════════╝
...
local function mkF(parent,bg,zi)   -- plain Frame
    local f=Instance.new("Frame")
    f.BackgroundColor3=bg or C.BG; f.BorderSizePixel=0
    if zi then f.ZIndex=zi end; f.Parent=parent; return f
end

local function mkL(parent,txt,sz,col,font,xa)  -- TextLabel
...
-- ══════════════════════════════════════════════════════════════════════════════
-- WINDOW
-- Key insight: Win has ClipsDescendants=true with corner radius R.
-- The "border" is a frame BEHIND Win with the SAME corner radius R,
-- sized 2px larger on each side → looks like a 1px border, perfectly rounded.
-- ══════════════════════════════════════════════════════════════════════════════
local R = 12   -- single source of truth for corner radius

local BG = mkF(Screen, C.Border)   -- border layer
BG.Size=UDim2.new(0,662,0,422); BG.Position=UDim2.new(0.5,-331,0.5,-211)
rnd(BG, R)   -- SAME radius as Win

local Win = mkF(Screen, C.BG)
Win.Name="Win"; Win.Size=UDim2.new(0,660,0,420)
Win.Position=UDim2.new(0.5,-330,0.5,-210)
Win.ClipsDescendants=true   -- clips sidebar square corners → rounded
rnd(Win, R)
...
Dot.Size=UDim2.new(0,7,0,7); Dot.Position=UDim2.new(0,16,0.5,-3); rnd(Dot,4)

local TitleL = mkL(Top,"Leon X",15,C.Text,Enum.Font.GothamBold)
TitleL.Size=UDim2.new(0,80,1,0); TitleL.Position=UDim2.new(0,30,0,0)

local VerL = mkL(Top,"v4.0",11,C.Sub,Enum.Font.Gotham)
VerL.Size=UDim2.new(0,40,1,0); VerL.Position=UDim2.new(0,110,0,0)

-- ── Window buttons (on Screen, tracked to Win) ────────────────────────────────
local function mkWinBtn(icon,bg)
    local b=Instance.new("TextButton")
    b.Size=UDim2.new(0,26,0,26); b.BackgroundColor3=bg
    ...
    b.ZIndex=5; b.Parent=Screen; rnd(b,7)
    return b
end
...
SideScroll.Parent=Sidepdg(SideScroll,10,0,0,10)

local SideLL=Instance.new("UIListLayout")
SideLL.Padding=UDim.new(0,4); SideLL.HorizontalAlignment=Enum.HorizontalAlignment.Center
SideLL.SortOrder=Enum.SortOrder.LayoutOrder; SideLL.Parent=SideScroll
...
Content.Size=UDim2.new(1,-153,1,-46); Content.Position=UDim2.new(0,153,0,46)

local Pages={}; Library._first=true
...
ResBtn.ZIndex=5; 
ResBtn.Parent=Screen; rnd(ResBtn,5)
...
Float.Visible=false; Float.ZIndex=5; 
Float.Parent=Screen
rnd(Float,12); strk(Float)
...
do
    ...
    UIS.InputEnded:Connect(function(i)
        if i.UserInputType==Enum.UserInputType.MouseButton1 and on then
            on=false; if not mv then showWin() end; task.wait(); mv=false
        end
    end)
end
...
local function mkSection(parent, name)
    ...
    local t=mkL(pill,name,10,C.Sub,
...
local function mkToggle(parent, data)
	...
	track.Position=UDim2.new(1,-52,0.5,-10); rnd(track,10)
...
local function mkSlider(parent, data)
    ...
    w.Size=UDim2.new(1,0,0,54); rnd(w,10); strk(w)
    ...
    trackBg.Size=UDim2.new(1,-28,0,4); trackBg.Position=UDim2.new(0,14,1,-16); rnd(trackBg,2)

    local fill=mkF(trackBg,C.Accent)
    fill.Size=UDim2.new((cur-mn)/(mx-mn),0,1,0); rnd(fill,2)

    local knob=mkF(trackBg,C.Accent)
    knob.Size=UDim2.new(0,12,0,12); knob.AnchorPoint=Vector2.new(0.5,0.5)
    knob.Position=UDim2.new((cur-mn)/(mx-mn),0,0.5,0); knob.ZIndex=3; rnd(knob,6)
...
-- Dropdown ────────────────────────────────────────────────────────────────────
-- List parented to Screen (escapes Win's ClipsDescendants).
-- NO ClipsDescendants on List — items are hidden via Visible, not height clip.
-- Height animates via Size tween; items inside are always fully rendered.
local function mkDropdown(parent, data)
    ...
    -- header row
    local w=mkF(parent,C.Elevated)
    w.Size=UDim2.new(1,0,0,42); rnd(w,10); strk(w)

    local hdr=Instance.new("TextButton")
    hdr.Size=UDim2.new(1,0,1,0); hdr.BackgroundTransparency=1
    hdr.Text=""; hdr.AutoButtonColor=false; hdr.BorderSizePixel=0; hdr.Parent=w

    local nl=mkL(hdr,data.Name or "Dropdown",13,C.Text,Enum.Font.GothamMedium)
    nl.Size=UDim2.new(1,-110,1,0); nl.Position=UDim2.new(0,14,0,0)

    local vl=mkL(hdr,cur,12,C.Dim,Enum.Font.Gotham,Enum.TextXAlignment.Right)
    vl.Size=UDim2.new(0,90,1,0); vl.Position=UDim2.new(1,-106,0,0)
    ...
    -- floating list on Screen — NOT ClipsDescendants
    local itemH=34
    local fullH=math.min(#opts*itemH+8, 180)

    local List=mkF(Screen,C.Surface)
    List.Size=UDim2.new(0,10,0,0)   -- collapsed
    List.ClipsDescendants=false      -- ← no clipping; items hidden via Visible
    List.Visible=false; List.ZIndex=20
    rnd(List,10); strk(List)

    -- items container (clips to list bounds for clean scroll)
    local ItemClip=mkF(List,C.Surface)
    ItemClip.BackgroundTransparency=1
    ItemClip.Size=UDim2.new(1,0,1,0)
    ItemClip.ClipsDescendants=true   -- clips items to current List height

    local ll=Instance.new("UIListLayout")
    ll.SortOrder=Enum.SortOrder.LayoutOrder; ll.Parent=ItemClip
    pdg(ItemClip,4,0,0,4)

    for _,opt in ipairs(opts) do
        local it=Instance.new("TextButton")
        it.Size=UDim2.new(1,0,0,itemH); it.BackgroundColor3=C.Surface
        it.Text=""; it.AutoButtonColor=false; it.BorderSizePixel=0
        it.ZIndex=21; it.Parent=ItemClip
        local il=mkL(it,opt,12,C.Text,Enum.Font.Gotham)
        il.Size=UDim2.new(1,-20,1,0); il.Position=UDim2.new(0,12,0,0); il.ZIndex=21
    ...
    local function openDD()
        open=true
        local ap=w.AbsolutePosition; local as=w.AbsoluteSize
    ...
    UIS.InputBegan:Connect(function(i)
        ...
        if outside(List) and outside(w) then closeDD() end
    end)
...
local function mkKeybind(parent, data)
    ...
    row.Size=UDim2.new(1,0,0,42); rnd(row,10); strk(row)

    local nl=mkL(row,data.Name or "Keybind",13,C.Text,Enum.Font.GothamMedium)
    nl.Size=UDim2.new(1,-110,1,0); nl.Position=UDim2.new(0,14,0,0)

    local kb=Instance.new("TextButton")
    kb.Size=UDim2.new(0,78,0,26); kb.Position=UDim2.new(1,-88,0.5,-13)
    kb.BackgroundColor3=C.Surface
    kb.Text=cur==Enum.KeyCode.Unknown and "None" or cur.Name
    kb.TextColor3=C.Dim; kb.Font=Enum.Font.GothamMedium; kb.TextSize=11
    kb.AutoButtonColor=false; kb.BorderSizePixel=0; kb.Parent=row
    rnd(kb,7); strk(kb)
...
local function mkLabel(parent, data)
    ...
    l.Size=UDim2.new(1,0,0,24); l.BorderSizePixel=0
    local api={Frame=l}
    function api:Set(t) l.Text=t end
    return api
end

-- ColorPicker (simple hue strip) ──────────────────────────────────────────────
local function mkColorPicker(parent, data)
    ...
    swatch.Size=UDim2.new(0,22,0,22); swatch.Position=UDim2.new(1,-36,0,8); rnd(swatch,6)

    -- hue bar
    local hueBar=mkF(w,C.OffTrack)
    hueBar.Size=UDim2.new(1,-28,0,6); hueBar.Position=UDim2.new(0,14,1,-18); rnd(hueBar,3)
    ...
    cursor.Position=UDim2.new(0,0,0.5,0); cursor.ZIndex=3; rnd(cursor,4)
...
function Library:CreateTab(name)
    ...
    btn.Size=UDim2.new(1,-12,0,36); btn.BackgroundColor3=C.Surface
    btn.Text=""; btn.AutoButtonColor=false; btn.BorderSizePixel=0; btn.Parent=SideScroll
    rnd(btn,9)

    local bar=mkF(btn,C.Accent)
    bar.Size=UDim2.new(0,3,0,18); bar.Position=UDim2.new(0,0,0.5,-9)
    bar.BackgroundTransparency=1; rnd(bar,2)

    local bl=mkL(btn,name,13,C.Sub,Enum.Font.GothamMedium)
    bl.Size=UDim2.new(1,-14,1,0); bl.Position=UDim2.new(0,14,0,0)

    -- scrolling page inside Content
