...
-- UI: ui/library.lua (Library:CreateTab / Tab:AddToggle / Tab:AddSlider etc.)
...
local function load(p) return loadstring(game:HttpGet(BASE..p.."?t="..os.time()))() end
...
Library:SetSplashProgress(0.08)
...
local PerfStats   = load("modules/visuals/perfstats.lua");      Library:SetSplashProgress(0.55)
local RemoveFog   = load("modules/visuals/removefog.lua");      Library:SetSplashProgress(0.58)
local AntiAFK     = load("modules/player/antiafk.lua");         Library:SetSplashProgress(0.62)
local InfStamina  = load("modules/player/infinitestamina.lua"); Library:SetSplashProgress(0.65)
local AntiFling   = load("modules/player/antifling.lua");       Library:SetSplashProgress(0.68)
local Rejoin      = load("modules/player/rejoin.lua");          Library:SetSplashProgress(0.71)
local Teleport    = load("modules/player/teleport.lua");        Library:SetSplashProgress(0.74)
local GodMode     = load("modules/player/godmode.lua");         Library:SetSplashProgress(0.77)
local NoFallDmg   = load("modules/player/nofalldamage.lua");    Library:SetSplashProgress(0.80)

ConfigMgr:Init(Library)
Library:SetSplashProgress(0.85)

-- ── Notification helper ───────────────────────────────────────────────────────
local function N(t, m, k, d)
    Library:Notify({ Title=t, Text=m, Type=k or "info", Duration=d or 2 })
end
...
Library:SetSplashProgress(0.90)
...
Mov:AddToggle({ Name="Speed Hack", Flag="SpeedHack", Default=false,
    Callback=function(v)
        ...
        N("Speed Hack", v and "Enabled" or "Disabled", v and "success" or "info")
    end })
...
Mov:AddToggle({ Name="Click Teleport", Flag="ClickTP", Default=false,
    Callback=function(v)
        if v then ClickTP:Enable() else ClickTP:Disable() end
        N("Click Teleport", v and "Enabled — click/tap to tp" or "Disabled", v and "success" or "info")
    end })
...
Set:AddToggle({ Name="Show Notifications", Flag="ShowNotifs", Default=true,
    Callback=function(v)
        Library:Notify({ Title="Notifications",
...
local nameIn = Set:AddTextInput({ Name="Config Name", Placeholder="e.g. pvp", Default="default",
...
local cfgDrop = Set:AddDropdown({ Name="Select Config",
    Options=cfgList(), Default=ConfigMgr:List()[1] or "(none)" })
...
Set:AddButton({ Name="💾  Save", Callback=function()
    ...
    cfgDrop:SetOptions(cfgList()); if ok then cfgDrop:Set(n) end
end })

Set:AddButton({ Name="📂  Load", Callback=function()
    local s=cfgDrop:Get(); if s=="(none)" then return end
    local ok=ConfigMgr:Load(s)
    N("Config", ok and "Loaded: "..s or "Load failed", ok and "success" or "error", 3)
end })

Set:AddButton({ Name="🗑  Delete", Callback=function()
    local s=cfgDrop:Get(); if s=="(none)" then return end
    ConfigMgr:Delete(s); N("Config","Deleted: "..s,"info",3)
...
Set:AddButton({ Name="⭐  Set as Default", Callback=function()
    ...
    N("Config", ok and s.." is default" or "Failed", ok and "success" or "error", 3)
end })

Set:AddSection("About")
Set:AddLabel({ Text="Leon X  ·  v2.0", Color=Color3.fromRGB(70,70,70),
