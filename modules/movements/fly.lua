-- Leon X | Fly Module
-- BodyVelocity + BodyGyro approach — smooth, no jitter, works in most games
...
local conn         -- RenderStepped connection
local keysDown = {}
...
function Fly:Enable()
    ...
    conn = RunService.RenderStepped:Connect(function()
        ...
        -- build direction from held keys relative to camera
        ...
        for key, vec in pairs(KEYS) do
            if UIS:IsKeyDown(key) then
                if vec.Y ~= 0 then
                    -- up/down is world-space
                    dir = dir + vec
                else
                    -- forward/back/left/right relative to camera yaw
        ...
        -- keep gyro facing camera direction (yaw only)
